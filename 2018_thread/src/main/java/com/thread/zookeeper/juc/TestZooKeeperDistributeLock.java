...
public class TestZooKeeperDistributeLock {
	...
	private void init() {
		try {
			...
			String address = "172.16.194.58:2181";
	...
	public static void main(String[] args) {
		...
		final TestZooKeeperDistributeLock testZooKeeperDistributeLock = new TestZooKeeperDistributeLock(true);
				...
				public void run() {
					...
					while (true) {
						try {
							...
							testZooKeeperDistributeLock.getZk().create("/tmp_root_path/lock",
							...
							Thread.currentThread().sleep(3000 + radom.nextInt(3000));
							...
							if (testZooKeeperDistributeLock.isReentrant()) {
								try {
									String lockThread = new String(testZooKeeperDistributeLock.getZk()
											.getData("/tmp_root_path/lock", false, null));
									if (lockThread != null) {

										// 当前线程与获取到的锁线程名一致，重入锁
										if (lockThread.equals(Thread.currentThread().getName())) {
								...
								System.out.println("线程" + Thread.currentThread().getName() + "尝试获取锁失败！");
							}
							...
							try {
								...
								if (locked) {
									...
									testZooKeeperDistributeLock.getZk().delete("/tmp_root_path/lock", -1);
